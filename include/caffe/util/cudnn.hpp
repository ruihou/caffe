...
inline void setTensorNdDesc(cudnnTensorDescriptor_t* desc,
  ...
  CHECK_EQ(shape.size(), stride.size())
      << "Dimensions of shape and stride do not match!";
  // Fill shape with 1 to create tensors with at least 4 dimensions
  // to prevent CUDNN_STATUS_BAD_PARAM eroor in CUDNN_v4.
...
inline void setNdConvolutionDesc(cudnnConvolutionDescriptor_t* conv,
  ...
  CUDNN_CHECK(cudnnGetFilterNdDescriptor(filter, shape.size(),
      &cudnn_type, &tensor_format, &nbDims, shape.data()));
#else
  CUDNN_CHECK(cudnnGetFilterNdDescriptor(filter, shape.size(),
      &cudnn_type, &nbDims, shape.data()));
#endif
...
#if CUDNN_VERSION_MIN(5, 0, 0)
  CUDNN_CHECK(cudnnSetConvolutionNdDescriptor(*conv, pad.size(),
      pad.data(), stride.data(), upscale.data(), CUDNN_CROSS_CORRELATION,
      cudnn_type));
#else
  CUDNN_CHECK(cudnnSetConvolutionNdDescriptor(*conv, pad.size(),
      pad.data(), stride.data(), upscale.data(), CUDNN_CROSS_CORRELATION));
